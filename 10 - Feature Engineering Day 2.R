...
df %>% 
  summarize(
    count_age_past_99 = sum(age > age_cap),
    count_sib_sp_past_99 = sum(sib_sp > sib_sp_cap),
    count_parch_past_99 = sum(parch > parch_cap),
    count_fare_past_99 = sum(fare > fare_cap)
  )


# cap fare, and check work before saving
df %>% 
